...
def main():   
    ...
    #Check what Mbot is running the code 
    for l in range(len(z0)):
        if z0[l] != 0.001:
            continue
        else:
    ...
    sigma_error = var_strings_.sigma_error

    
    n_agents = 5 # 1 Anchor + 4 Mbots moving
    
    x_input = np.array([[1,1],[2,3],[4,5],[3,2]]) # valori casuali per inizializzare e basta
    ...
    last_fraction_2 = time.time() # for the WLS
    num1 = 0
    while True:
        ...
        try:
            with open("distances_vector.pkl", 'rb') as file: # mi aspetto file sia un vettore
                ...
                distance_3 = dist_vec[3]
                distance_4 = dist_vec[4]
                dist_vec = np.array([distance_0, distance_1, distance_2, distance_3, distance_4])
        except FileNotFoundError:
            print("File not found.")
        except EOFError:
            print("No data found in the file.")    
        ...
        if subscriber_0 in socks and socks[subscriber_0] == zmq.POLLIN:
        ...
        if subscriber_1 in socks and socks[subscriber_1] == zmq.POLLIN:
        ...
        if subscriber_2 in socks and socks[subscriber_2] == zmq.POLLIN:
        ...
        if subscriber_3 in socks and socks[subscriber_3] == zmq.POLLIN:
        ...
        if subscriber_4 in socks and socks[subscriber_4] == zmq.POLLIN:
        ...
        if dt_2 >= 0.5:  
            ...
            '''
            if num1 % 10 == 0: #every 10 iteration reinitialize, are 2.5 seconds 
            
                x_est, P = wls_1.initialization(x_input_est, dist_vec)
            '''
            last_fraction_2 = time.time()
