...
//VARIABLES
  ...
  float power = 40.0/100; // PRIMO TENTATIVO VEDI VELOCITY PER CORREGGERE VELOCITA'
  ...
  */
  float steeringTime = 30000.0*1.13/(280*power); //tempo di rotazione a 30 gradi
  float backTime = 150*1000.0*1.13/(362*power); //RITORNO INDIETRO DI 100 mm 550
  float stopTime = 400*power; //TEMPO TRA STOP E RIAVVIO MOTORI
  ...
  float velocity = 30;//75*power; //cm/s
  float accSensitivity = 0.4;//0.23;//0.16
  float speSensitivity = 0.16;//0.12
  int collision=0;
  int nStop=0;
  int n = 0;
  int s = 0;
...
void setup(void) {
  ...
  while(s<2*dim){
    ...
    if(s>2*dim&&s<=8*dim&&s%6==0){
      power*=(1+(velocity-speMY)/velocity);
      motorSpeed = 255*power;
      steeringTime = 30000.0*1.13/(280*power); //tempo di rotazione a 30 gradi
      backTime = 150*1000.0*1.13/(362*power); //RITORNO INDIETRO DI 100 mm 550
      stopTime = 400*power; //TEMPO TRA STOP E RIAVVIO MOTORI
    }
    delay(sampTime);
    s++;
...
void motion_loop(){
  ...
  if(accT<3&&movement!=0){ 
    ...
    if(sqrt(angY*angY)>30||accT>accSensitivity*power*100||nStop>600/sampTime){
      collision=1;
      n=0;
    }else{
      collision==0;
  ...
  switch (movement) {
    ...
    case 2:   // Serial.println("right");
      motor1.setPWM(-motorSpeed*0.6*compsx);
      motor2.setPWM(motorSpeed*0.6*compdx);
      distStop = 180*power;
      break;
