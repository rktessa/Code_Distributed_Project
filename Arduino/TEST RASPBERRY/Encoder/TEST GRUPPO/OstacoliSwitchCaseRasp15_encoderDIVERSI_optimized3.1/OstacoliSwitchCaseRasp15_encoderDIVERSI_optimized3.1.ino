...
void motion_loop() {
  ...
  if (s > dim) {
    ...
    if (sqrt(angY * angY) > 30 || accT > accSensitivity * power * 100 || nStop > 800 / sampTime|| distnow <= 75*power) {
      if (speMY >= 0) {
        collision = 1;
        n = 0;
      }
  ...
  switch (movement) {
    ...
    case 2:  // Serial.println("right");
      motor1.run(motorSpeed * 0.7 * compsx);
      motor2.run(-motorSpeed * 0.7 * compdx);
      distStop = 300 * power;
      break;
