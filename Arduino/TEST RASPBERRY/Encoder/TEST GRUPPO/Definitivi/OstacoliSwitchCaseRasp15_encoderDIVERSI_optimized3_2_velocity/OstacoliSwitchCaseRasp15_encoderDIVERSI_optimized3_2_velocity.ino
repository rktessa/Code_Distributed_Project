...
const float compdx = 98.0 / 100;
const float wheel = 64.0 * PI;  //circonference
...
const float accSensitivity = 0.6;  //0.23;//0.16   0.0 - 1.0
const float speSensitivity = 0.1;  //0.12  % della velocity
int collision = 0;
int nStop = 0;
int steer = 2;
int n = 0;
...
void motion_loop() {
  ...
  if (s > dim) {
    ...
    if (sqrt(angY * angY) > 30 || accT > ((1.5 - accSensitivity) * 10 + power * 2) && abs(accY) > 3 || nStop > 1000 / sampTime || distnow <= 75 * power) {
      if (speMY >= 0) {
        collision = 1;
        n = 0;
        if (rotZ > 0) {
...
void read_quadrature() {
  ...
  if (digitalRead(ENCODER_A_PIN) == LOW) {
    if (digitalRead(ENCODER_B_PIN) == LOW) { pulse_number--; }
    if (digitalRead(ENCODER_B_PIN) == HIGH) { pulse_number++; }
  }
  //return pulse_number;
}
...
void read_quadrature2() {
  ...
  if (digitalRead(ENCODER_A2_PIN) == LOW) {
    if (digitalRead(ENCODER_B2_PIN) == LOW) { pulse_number2--; }
    if (digitalRead(ENCODER_B2_PIN) == HIGH) { pulse_number2++; }
  }
  //return pulse_number;
}
