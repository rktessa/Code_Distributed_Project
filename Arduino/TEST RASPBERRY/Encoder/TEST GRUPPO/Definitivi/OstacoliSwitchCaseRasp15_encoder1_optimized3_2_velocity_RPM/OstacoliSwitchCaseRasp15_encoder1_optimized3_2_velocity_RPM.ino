...
uint8_t motorSpeed = 200 * power;                      // max 200 rpm
...
void setup(void) {
  ...
  motor1.runSpeed(motorSpeed * compsx);
  motor2.runSpeed(motorSpeed * compdx);
  delay(2000);
...
void motion_loop() {
  ...
  switch (movement) {
    ...
    case 2:  // Serial.println("right");
      motor1.runSpeed(motorSpeed * 0.8 * compsx);
      motor2.runSpeed(-motorSpeed * 0.8 * compdx);
      distStop = 300 * power;
      break;
