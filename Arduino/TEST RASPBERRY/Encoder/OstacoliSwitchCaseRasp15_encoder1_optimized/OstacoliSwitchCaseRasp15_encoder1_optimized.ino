...
*/
const float power = 40.0/100; // 50 % potenza
...
const float backTime = 150*1000.0*1.13/(362*power); //RITORNO INDIETRO DI 100 mm 550
...
*/
  ...
  //rotMZ = (speM1-speM2)*10/halfinteraxis;
  //posM1 += speM1*cycleTime/1000;
  //posM2 += speM2*cycleTime/1000;

  if(abs(speMY)<speSensitivity*velocity){
      nStop++;
      }else{
      nStop=0;
    }
  /*
    if(sqrt((speStop-speT)*(speStop-speT))<speSensitivity*velocity||abs(speMY)<speSensitivity*velocity){
      nStop++;
      }else{
      speStop=speT;
      nStop=0;
...
if(collision==0){
    ...
    if(sqrt(angY*angY)>30||accT>accSensitivity*power*90||nStop>600/sampTime){
    collision=1;
      n=0;
    }
