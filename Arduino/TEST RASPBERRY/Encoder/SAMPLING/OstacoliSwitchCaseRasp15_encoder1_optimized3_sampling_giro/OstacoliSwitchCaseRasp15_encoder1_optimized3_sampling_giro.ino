...
float power = 25.0 / 100;         // PRIMO TENTATIVO VEDI VELOCITY PER CORREGGERE VELOCITA'
float motorSpeed = 255 * power;   /* value: between -255 and 255. */
...
/*MBOT MCORE
  ...
  */
const float steeringTime = 3000;                      //3*360 in 6 s                     //30000.0*1.13/(280*power); //tempo di rotazione a 30 gradi
const float backTime = 100 * 1000.0 / (750 * power);  //RITORNO INDIETRO DI 100 mm 550
const float stopTime = 400 * 1.13 * power;            //TEMPO TRA STOP E RIAVVIO MOTORI
const float forwTime = 300 * 1000.0 / (750 * power);  //AVANTI di 1000 mm
const int sampTime = 25;                              //tempo in millis tra una lettura e l'altra
const int delaysampTime = 500;
long start = 0;
long cycleTime = 0;  //tempo per compiere un ciclo loop o tempo di campionatemento

float distStop = 100 * power;       //cm
const float velocity = 75 * power;  //cm/s 122 cm/s al 100% con 7.89V
...
void setup(void) {
  ...
  */
  ...
  motor1.setPWM(0);
  motor2.setPWM(0);
  Serial.println("Calibration power done");
...
void motion_loop() {
  ...
  */
  ...
  if (n * cycleTime > steeringTime && s < dim) {
    rotZ /= n;
    speM1 /= n;
    speM2 /= n;

    ArotZ[s] = rotZ;
    AspeM1[s] = wheel * 2 * speM1 / (60 * halfinteraxis);
    AspeM2[s] = wheel * 2 * speM1 / (60 * halfinteraxis);
    ...
    Serial.print(accY);
    Serial.print(",");
    Serial.print(power);  //accZ
    Serial.print(",");
    erial.print(rotX);  //rotX
    Serial.print(",");
  ...
  if (n * cycleTime >= delaysampTime && n * cycleTime <= (steeringTime - delaysampTime)) {
    rotZ += g.gyro.z - rotZError;
    speM1 += -motor1.getCurrentSpeed();  //*wheel/600;
    speM2 += motor2.getCurrentSpeed();   //*wheel/600;
  }
