...
const float power = 40.0 / 100;   // PRIMO TENTATIVO VEDI VELOCITY PER CORREGGERE VELOCITA'
...
/*MBOT MCORE
  ...
  */
const float steeringTime = 700;        //3*360 in 6 s                     //30000.0*1.13/(280*power); //tempo di rotazione a 30 gradi
  ...
  */
  ...
  if (n * cycleTime > forwTime) {                                  //forward
    if ((n * cycleTime - forwTime) > steeringTime) {                   //right
      if ((n * cycleTime - forwTime - steeringTime) > forwTime/2) {  //forward
        if ((n * cycleTime - forwTime*3/2 - steeringTime) > steeringTime) {  //left
          if ((n * cycleTime - forwTime*3/2 - 2*steeringTime) > forwTime) { //forward
            movement = 0;
          } else {
            movement=1;
          }
