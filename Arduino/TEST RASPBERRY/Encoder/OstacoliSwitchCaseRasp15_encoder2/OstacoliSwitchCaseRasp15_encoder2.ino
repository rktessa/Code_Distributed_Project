...
//float AspeM1[dim]; //array speM1
//float AspeM2[dim]; //array speM2
...
while(1){
  ...
  if(data == Start){
    ...
    Serial.println("The run starts in 4 seconds");
    delay(4000);
    Serial.println(data);
    delay(10);
    break;
...
void motion_loop(){
  ...
  //AspeM1[0] = -motor1.getCurrentSpeed()*wheel/600;
  //AspeM2[0] = motor2.getCurrentSpeed()*wheel/600;

  //speM1 = AspeM1[0] - AspeM1[dim-1]/(dim-1);
  //speM2 = AspeM2[0] - AspeM2[dim-1]/(dim-1);
  ...
  for(int i=0;i<(dim-2);i++){
    ...
    //AspeM1[dim - 1 - i] = AspeM1[dim - 2 - i] + AspeM1[0];
    //AspeM2[dim - 1 - i] = AspeM2[dim - 2 - i] + AspeM2[0];
   }
  ...
  posM1 += (-motor1.getCurrentSpeed()*wheel/600-speM1)*cycleTime/1000;
  posM2 += (motor2.getCurrentSpeed()*wheel/600-speM2)*cycleTime/1000;
...
if(collision==0){
    ...
    if(sqrt(angY*angY)>30||accT>accSensitivity*power*resamotori||nStop>600/sampTime){
    collision=1;
      n=0;
    }
