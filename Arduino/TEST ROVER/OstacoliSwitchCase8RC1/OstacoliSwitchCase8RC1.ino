...
void setup(void) {
  ...
  while(analogRead(A7) > 10){
    ...
    rgbled_7.show();
     ...
     if(ir.keyPressed(69)){//PREMENDO TAST A FA PARTIRE
         break;
      }
    n++;
    delay(100);
...
void loop() {
    ...
    if(analogRead(A7) < 10||ir.keyPressed(71)){
      motor1.stop();
       motor2.stop();
    resetFunc();
  }
...
if(ir.keyPressed(69)){//PREMENDO TAST A ENTRI IN MANUALE
  ...
  while(1){
    ...
    if(ir.keyPressed(7)){//PREMENDO TAST sx SINISTRA
      movement = 4;
       }
    if(ir.keyPressed(9)){//PREMENDO TAST dx DESTRA
      movement = 2;
       }
     if(ir.keyPressed(68)){//PREMENDO TAST D DIMINUISCI VELOCITA
      motorSpeed -= 10;
      delay(100);
       }
    if(ir.keyPressed(67)){//PREMENDO TAST E AUMENTI VELOCITA
      motorSpeed += 10;
      delay(100);
       }
