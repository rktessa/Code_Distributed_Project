...
def bot_parser(bot_l, data):
        global bot_name, ard_com_time, READY, ready_c, tmp_time, error_counter, error_counter1, good_data, client, logger_i, LOG_DATA,SHOW_DATA, server_1,name_dir,count_msg2,count_msg, timer,new_time, imu_data, imu_trigger,t_epoch,trig
        try:
                        ...
                        if data[1] == b'd':			
                               ...
                               # print(bcolors.OKGREEN+str(imu_trigger)+bcolors.ENDC)
                                if imu_trigger >= 0.1:#send data to simulink
                                        ...
                                        encoded = data_encoding(accx,accy,omega,t_acq-ard_com_time)
								...
								#log input data
                                if LOG_DATA:
                                        logger_i.imu(accx,accy, acc, omega, t_acq-ard_com_time, trig)
                                        trig = 0
...
def client_parser(data): 
	global bot, is_point_to_point, ask_go, is_stop_bot, error_counter1, good_data, client, bot_name, name_dir, logger_i,logger_c,logger_f,logger_t, LOG_DATA, SHOW_DATA, server_1,count_msg2,count_msg, timer,new_time, imu_data, imu_trigger,t_epoch,ard_com_time
	try:
		...
		#camera data parser --> data coming from camera and processed by the server and send to the bot                 
			...
			if imu_trigger > 0.05 :
				...
				encoded1 = data_encoding(imu_data[0],imu_data[1],imu_data[2],t_acq-ard_com_time)
				server_5.send_data2(encoded1)
				#if LOG_DATA:
					#logger_i.imu(imu_data[0],imu_data[1], acc, imu_data[2], t_acq-ard_com_time)
			cam = detected_bot.from_server_string(data[1:])
			delay = time.time()-cam.timestamp
			#print(bcolors.OKBLUE+"SEND CAM" +bcolors.ENDC)
			encoded = data_encoding(cam.poseX,cam.poseY,cam.orientation,t_acq-delay)
			...
			if LOG_DATA:
				logger_c.camera(cam.poseX, cam.poseY, cam.orientation, t_acq, t_acq-delay)   
...
def feedback_control(x,y,phi,v,omega,t):
	...
	try:
		...
		if LOG_DATA:
			...
			logger_f.control(x,y,phi,v,omega,t_acq)
			t_elapsed = time.time()
...
if __name__== "__main__":
	try:
		...
		server_3.set_parser(feedback_control)
