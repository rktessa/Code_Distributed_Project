		...
		while True:
			try:
				...
				if conv_est!= b'\x7f\xf8\x00\x00\x00\x00\x00\x00\x7f\xf8\x00\x00\x00\x00\x00\x00':
					...
					p[0] = struct.unpack('>d', struct.pack('8B',*conv_est[40:48]))[0]
					p[1] = struct.unpack('>d', struct.pack('8B',*conv_est[48:56]))[0]
					p[2] = struct.unpack('>d', struct.pack('8B',*conv_est[56:64]))[0]
					p[3] = struct.unpack('>d', struct.pack('8B',*conv_est[64:72]))[0]
					p[4] = struct.unpack('>d', struct.pack('8B',*conv_est[72:80]))[0]
					
					p[5] = struct.unpack('>d', struct.pack('8B',*conv_est[80:88]))[0]
					p[6]= struct.unpack('>d', struct.pack('8B',*conv_est[88:96]))[0]
					p[7] = struct.unpack('>d', struct.pack('8B',*conv_est[96:104]))[0]
					p[8] = struct.unpack('>d', struct.pack('8B',*conv_est[104:112]))[0]
					

					
					#print('p:'+str(p[0]))
					t = time.time()
					self.parser2(x,y,phi,v,omega,t)
					self.parser(p,t)
