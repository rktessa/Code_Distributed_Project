...
import sys
import select
import global_variable_
from central_pub import *
...
def main():
    while True:
        ...
        if command == 'inizio':
            global_variable_.stato = 2 #Initially set to reset
            while True:
                ...
                # Continua l'esecuzione del loop interno
                    ...
                    global_variable_.stato = 2
                    pubblish_global_state()
                elif nuovo_input == calibrate:
                    global_variable_.stato = 3
                    pubblish_global_state()
                elif nuovo_input == 'fine':
                    global_variable_.stato = 2 # Manda un reset un ultima volta
                    pubblish_global_state()
                    break
                else:
