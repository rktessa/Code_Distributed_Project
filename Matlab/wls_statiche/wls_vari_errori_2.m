...
for mag = 0: (size_mag-1)
    ...
    wls_2 = wls_class(anchors, sigma_error,n_agents,x_input, mbot_posizione_vera2, reference_positon);
    ...
    dist2 = wls_2.distance(x_input,mbot_posizione_vera2); %inizializzo la prima
    z02 = wls_2.z_sensor(x_input, dist2);
    [x_est2, P2] = wls_2.initialization(x_input,z02);
    
    
    sol(1,:, mag+1)= [x_est(2),x_est(3), x_est2(2),x_est2(3)];
    ...
    for i=2:iter        
        ...
        dist2 = wls_2.distance(x_input,mbot_posizione_vera2);
        [x_est2, P2] = wls_2.WLS3_distributed(x_est2, P2, x_input, dist2);  

        x_input = [x_est(2),x_est(3); x_est2(2),x_est2(3)];
        sol(i,:,mag+1) = [x_est(2),x_est(3), x_est2(2),x_est2(3)]; % salvo le soluzioni  
    end
    ...
    E_sigma(mag+1,3) = sqrt(mean((A1 - F1).^2)); %Root Mean Square Error
    E_sigma(mag+1,4) = sqrt(mean((A2 - F2).^2));
    V_sigma(mag+1,3) = var(F1);
    V_sigma(mag+1,4) = var(F2);
...
%Plot initial position estimation
    ...
    % plot(sol(:,1,2), 'linewidth',1.0 )
    % plot(sol(:,1,3), 'linewidth',1.0 )
    plot(sol(:,3,4), 'linewidth',1.0 )
    % plot(sol(:,1,5), 'linewidth',1.0 )
    % plot(sol(:,1,6), 'linewidth',1.0 )
    plot(sol(:,3,7), 'linewidth',1.0 )
    % plot(sol(:,1,8), 'linewidth',1.0 )
    % plot(sol(:,1,9), 'linewidth',1.0 )
    plot(sol(:,3,10), 'linewidth',1.0 )
    ...
    % plot(sol(:,1,2), 'linewidth',1.0 )
    % plot(sol(:,1,3), 'linewidth',1.0 )
    plot(sol(:,4,4), 'linewidth',1.0 )
    % plot(sol(:,1,5), 'linewidth',1.0 )
    % plot(sol(:,1,6), 'linewidth',1.0 )
    plot(sol(:,4,7), 'linewidth',1.0 )
    % plot(sol(:,1,8), 'linewidth',1.0 )
    % plot(sol(:,1,9), 'linewidth',1.0 )
    plot(sol(:,4,10), 'linewidth',1.0 )
    ...
    plot(sigma_vec.^2,E_sigma(:,3), 'LineWidth',1.5)
    plot(sigma_vec.^2,E_sigma(:,4), 'LineWidth',1.5)
    ...
    plot(sigma_vec,V_sigma(:,3), 'LineWidth',1.5)
    plot(sigma_vec,V_sigma(:,4), 'LineWidth',1.5)
...
for e = 1:n
    ...
    wls_movement2 = wls_class(anchors, sigma_error,n_agents,x_input, x_mbot2, mbot_vera_motion);
    ...
    for l2=2:2000
        ...
        if mod(l2,20)== 0 %ogni x misure reinizializzo, 20 sembra un minimo con questa dinamica di Movimento di Mbot
            ...
            dist2 = wls_movement2.distance(x_input,x_mbot2); %reinizializzo
            z02 = wls_movement2.z_sensor(x_input, dist2);
            [x_est2, P2] = wls_movement2.initialization(x_input,z02);
        ...
        dist2 = wls_movement2.distance(x_input,x_mbot2);
        [x_est2, P2] = wls_movement2.WLS3_distributed(x_est2, P2, x_input, dist2);
